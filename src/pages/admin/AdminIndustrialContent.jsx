...
const PAGE = 'industrial';
const FOLDER = 'albloshi/industrial';

const TABS = [
  { key: 'hero', label: 'Hero Banner' },
  { key: 'stats', label: 'Stat Strip' },
  { key: 'products', label: 'Products' },
  { key: 'capabilities', label: 'Why Source From Us' },
...
// Editable list of plain-text spec lines for one language.
...
export default function AdminIndustrialContent() {
  ...
  // field on an item inside a rawList section (the section IS the array), e.g. sections.products[i].title_en
  ...
  const renderProducts = () => {
    ...
    return (
      <>
        <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: '0 0 1rem' }}>{sections.products.length} products. Specs are shown as chips on the product's "Read More" popup.</p>
  ...
  const renderCapabilities = () => {
    const lang = langs.capabilities;
    const sec = sections.capabilities;
    return (
      <>
        <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`title_${lang}`]} onChange={e => setField('capabilities', `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={2} value={sec[`desc_${lang}`]} onChange={e => setField('capabilities', `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
        <div style={grid2}>
          {sec.items.map((it, i) => (
            <div key={i} style={cardBox}>
              ...
              <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={it[`title_${lang}`]} onChange={e => setSubItemField('capabilities', 'items', i, `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
              <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={3} value={it[`desc_${lang}`]} onChange={e => setSubItemField('capabilities', 'items', i, `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
            </div>
          ))}
        </div>
      </>
    ...
    return (
      <>
        ...
        <div style={grid2}>
          ...
          <Field label="Button 2 Link" hint="e.g. /#segments or /industrial-services"><input value={sec.btn2_href} onChange={e => setField('cta', 'btn2_href', e.target.value)} style={inp()} {...focusHandlers} /></Field>
        </div>
      </>
    );
  };
  ...
  return (
    <>
      <Helmet><title>Industrial Services Content | Albloshi Admin</title><meta name="robots" content="noindex" /></Helmet>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <AdminLayout title="Industrial Services">
        {loading && (
