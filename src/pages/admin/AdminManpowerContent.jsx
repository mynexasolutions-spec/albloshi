...
const PAGE = 'manpower';
const FOLDER = 'albloshi/manpower';

const TABS = [
  { key: 'hero', label: 'Hero Banner' },
  { key: 'items', label: 'Manpower Categories' },
  { key: 'features', label: 'Why Choose Us' },
...
export default function AdminManpowerContent() {
  ...
  const renderHero = () => {
    ...
    return (
      <>
        <Field label="Background Image"><ImageField value={sec.image} onChange={v => setField('hero', 'image', v)} ratio="4 / 3" /></Field>
        <Field label={`Badge Text (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`badge_${lang}`]} onChange={e => setField('hero', `badge_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        <Field label={`Title Part 1 (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`title1_${lang}`]} onChange={e => setField('hero', `title1_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        <Field label={`Title Part 2 (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`title2_${lang}`]} onChange={e => setField('hero', `title2_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        ...
        <div style={grid2}>
          <Field label={`"Skilled" Filter Tab Label (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`skilled_label_${lang}`]} onChange={e => setField('hero', `skilled_label_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
          <Field label={`"Unskilled" Filter Tab Label (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`unskilled_label_${lang}`]} onChange={e => setField('hero', `unskilled_label_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        </div>
      </>
    );
  };
  ...
  const renderItems = () => {
    const lang = langs.items;
    return (
      <>
        <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: '0 0 1rem' }}>{sections.items.length} manpower categories.</p>
        {sections.items.map((it, i) => (
          <div key={it.id ?? i} style={cardBox}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '0.75rem', flexWrap: 'wrap', gap: 8 }}>
              <span style={{ fontSize: '0.78rem', fontWeight: 700, color: '#64748b' }}>Category {i + 1}</span>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                <select value={it.category} onChange={e => setListItemField('items', i, 'category', e.target.value)} style={{ ...inp({ width: 'auto', cursor: 'pointer' }) }}>
                  <option value="skilled">Skilled</option>
                  <option value="unskilled">Unskilled</option>
                </select>
                <ListItemControls index={i} length={sections.items.length}
                  onMoveUp={() => moveListItem('items', i, -1)}
                  onMoveDown={() => moveListItem('items', i, 1)}
                  onRemove={() => removeListItem('items', i)} />
              </div>
            </div>
            <div style={grid2}>
              <div>
                <Field label="Image (square)"><ImageField value={it.image} onChange={v => setListItemField('items', i, 'image', v)} ratio="1 / 1" /></Field>
              </div>
              <div>
                <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={it[`title_${lang}`]} onChange={e => setListItemField('items', i, `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
                <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={3} value={it[`desc_${lang}`]} onChange={e => setListItemField('items', i, `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
              </div>
            </div>
            <Field label={`Specifications (${lang.toUpperCase()})`}>
              <SpecList specs={it[`specs_${lang}`]} onChange={v => setListItemField('items', i, `specs_${lang}`, v)} />
            </Field>
          </div>
        ))}
        <button type="button" style={addBtnStyle} onClick={() => addListItem('items', {
          id: `item-${Date.now()}`, image: '', category: 'skilled', title_en: '', title_ar: '',
          desc_en: '', desc_ar: '', specs_en: [], specs_ar: [],
        })}>
          <span className="material-icons" style={{ fontSize: '1rem' }}>add</span> Add Manpower Category
        </button>
      </>
    );
  };
  ...
  const renderFeatures = () => {
    const lang = langs.features;
    const sec = sections.features;
    return (
      <div style={grid2}>
        {sec.items.map((it, i) => (
          <div key={i} style={cardBox}>
            ...
            <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={it[`title_${lang}`]} onChange={e => setSubItemField('features', 'items', i, `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
            <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={3} value={it[`desc_${lang}`]} onChange={e => setSubItemField('features', 'items', i, `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
          </div>
        ))}
      </div>
    );
  ...
  const renderCta = () => {
    ...
    return (
      <>
        ...
        <div style={grid2}>
          <Field label="Call Button Link" hint="e.g. tel:+966543188882"><input value={sec.btn1_href} onChange={e => setField('cta', 'btn1_href', e.target.value)} style={inp()} {...focusHandlers} /></Field>
          <Field label="WhatsApp Button Link" hint="e.g. https://wa.me/966543188882"><input value={sec.btn2_href} onChange={e => setField('cta', 'btn2_href', e.target.value)} style={inp()} {...focusHandlers} /></Field>
        </div>
        <p style={{ fontSize: '0.75rem', color: '#94a3b8' }}>Button labels ("Call"/"WhatsApp") are shared site-wide text, not editable here — only the links above.</p>
      </>
    );
  };
  ...
  return (
    <>
      <Helmet><title>Manpower Supply Content | Albloshi Admin</title><meta name="robots" content="noindex" /></Helmet>
      <style>{`@keyframes spin{to{transform:rotate(360deg)}}`}</style>
      <AdminLayout title="Manpower Supply">
        {loading && (
