...
const PAGE = 'about';
const FOLDER = 'albloshi/about';

const TABS = [
  { key: 'hero', label: 'Hero Banner' },
  { key: 'stats', label: 'Stats Strip' },
  { key: 'story', label: 'Our Story' },
  { key: 'mv', label: 'Mission, Vision & Promise' },
...
export default function AdminAboutContent() {
  ...
  // plain-array items (stats' `value`, story's `highlights` en/ar pairs)
      ...
      <div style={grid2}>
        {sections.stats.map((s, i) => (
          <div key={i} style={cardBox}>
            <Field label="Value (e.g. 8+)"><input value={s.value} onChange={e => setListItemField('stats', i, 'value', e.target.value)} style={inp()} {...focusHandlers} /></Field>
  ...
  const renderStory = () => {
    const lang = langs.story;
    const sec = sections.story;
    return (
      <>
        <Field label="Image"><ImageField value={sec.image} onChange={v => setField('story', 'image', v)} ratio="4 / 3" /></Field>
        <p style={{ fontSize: '0.75rem', color: '#94a3b8', margin: '0 0 0.85rem' }}>The CR number badge on the photo pulls the number from Contact &amp; Settings — only the "CR No." label text is edited here.</p>
        <div style={grid2}>
          <Field label={`Section Label (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`label_${lang}`]} onChange={e => setField('story', `label_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
          <Field label={`"CR No." Badge Label (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`cr_label_${lang}`]} onChange={e => setField('story', `cr_label_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        </div>
        <Field label={`Paragraph 1 (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={3} value={sec[`p1_${lang}`]} onChange={e => setField('story', `p1_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
        ...
        <div style={grid2}>
          <Field label={`Before (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`p2_before_${lang}`]} onChange={e => setField('story', `p2_before_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
          <Field label={`Bold phrase (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`p2_strong_${lang}`]} onChange={e => setField('story', `p2_strong_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        </div>
        <Field label={`After (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`p2_after_${lang}`]} onChange={e => setField('story', `p2_after_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>

        <p style={{ fontSize: '0.72rem', fontWeight: 700, color: '#94a3b8', textTransform: 'uppercase', letterSpacing: '0.05em', margin: '1.25rem 0 0.5rem' }}>Highlight checklist</p>
        {sec.highlights.map((h, i) => (
          <Field key={i} label={`Highlight ${i + 1} (${lang.toUpperCase()})`}>
            <input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={h[lang]} onChange={e => setSubItemField('story', 'highlights', i, lang, e.target.value)} style={inp()} {...focusHandlers} />
          </Field>
        ))}
      </>
    );
  ...
  const renderMv = () => {
    const lang = langs.mv;
    const sec = sections.mv;
    const cards = [
      { key: 'mission', icon: 'flag', label: 'Mission' },
      { key: 'vision', icon: 'visibility', label: 'Vision' },
      { key: 'promise', icon: 'star', label: 'Promise' },
    ];
    return (
      <div style={grid2}>
        {cards.map(c => (
          <div key={c.key} style={cardBox}>
            <div style={{ marginBottom: '0.6rem' }}><ReadOnlyBadge><span className="material-icons" style={{ fontSize: '0.9rem' }}>{c.icon}</span> {c.label} card</ReadOnlyBadge></div>
            <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`${c.key}_title_${lang}`]} onChange={e => setField('mv', `${c.key}_title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
            <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={4} value={sec[`${c.key}_desc_${lang}`]} onChange={e => setField('mv', `${c.key}_desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
          </div>
        ))}
      </div>
    );
  ...
  const renderValues = () => {
    const lang = langs.values;
    const sec = sections.values;
    return (
      <>
        <Field label={`Label (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`label_${lang}`]} onChange={e => setField('values', `label_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={sec[`title_${lang}`]} onChange={e => setField('values', `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
        <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={2} value={sec[`desc_${lang}`]} onChange={e => setField('values', `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
        <div style={grid2}>
          {sec.items.map((it, i) => (
            <div key={i} style={cardBox}>
              ...
              <Field label={`Title (${lang.toUpperCase()})`}><input dir={lang === 'ar' ? 'rtl' : 'ltr'} value={it[`title_${lang}`]} onChange={e => setSubItemField('values', 'items', i, `title_${lang}`, e.target.value)} style={inp()} {...focusHandlers} /></Field>
              <Field label={`Description (${lang.toUpperCase()})`}><textarea dir={lang === 'ar' ? 'rtl' : 'ltr'} rows={3} value={it[`desc_${lang}`]} onChange={e => setSubItemField('values', 'items', i, `desc_${lang}`, e.target.value)} style={inp({ resize: 'vertical' })} {...focusHandlers} /></Field>
            </div>
          ))}
        </div>
      </>
  ...
  return (
    <>
      <Helmet><title>About Page Content | Albloshi Admin</title><meta name="robots" content="noindex" /></Helmet>
